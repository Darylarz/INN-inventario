...
<div class="max-w-xl mx-auto">
  ...
  <form method="POST" action="{{ route('inventory.entrada.store', $inventory) }}" class="bg-white dark:bg-gray-800 shadow rounded p-4 space-y-4">
    @csrf
    <div>
      <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Cantidad a ingresar</label>
      <input type="number" name="quantity" min="1" required class="mt-1 w-full rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100" value="{{ old('quantity', 1) }}">
      @error('quantity')
