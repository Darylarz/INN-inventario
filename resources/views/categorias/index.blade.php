...
<div class="container mx-auto p-6">
  ...
  <div class="bg-white dark:bg-gray-800 rounded shadow overflow-hidden">
    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
      <thead class="bg-lime-100 dark:bg-lime-700">
        <tr>
        ...
        <tr>
          ...
          <td class="px-4 py-2 text-sm">
            ...
            <form method="POST" action="{{ route('categorias.deactivate', $c) }}" class="inline" onsubmit="return confirm('¿Desactivar categoría?');">
              @csrf
              <button
               class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500 text-white font-semibold hover:bg-red-600" style="color: white !important;">Desactivar</button>
            </form>
          </td>
        </tr>
        @empty
    ...
    <div class="px-4 py-3">{{ $categorias->links() }}</div>
  </div>
</div>
@endsection
