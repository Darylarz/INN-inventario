...
<div class="max-w-3xl mx-auto bg-white shadow rounded p-6 mt-6">
    <form action="{{ route('consumibles.actualizar', $consumible->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="categoria" value="{{ old('categoria', $consumible->categoria) }}" class="w-full border rounded px-2 py-1">
                @error('categoria') <span class="text-red-600">{{ $message }}</span> @enderror
            </div>

            <div>
                <label class="block font-medium">Marca</label>
                <input type="text" name="marca" value="{{ old('marca', $consumible->marca) }}" class="w-full border rounded px-2 py-1">
                @error('marca') <span class="text-red-600">{{ $message }}</span> @enderror
            </div>

            <div>
                <label class="block font-medium">Modelo</label>
                <input type="text" name="modelo" value="{{ old('modelo', $consumible->modelo) }}" class="w-full border rounded px-2 py-1">
                @error('modelo') <span class="text-red-600">{{ $message }}</span> @enderror
            </div>

            <div>
                <label class="block font-medium">Modelo de impresora / Color</label>
                <input type="text" name="modelo_impresora" value="{{ old('modelo_impresora', $consumible->modelo_impresora) }}" class="w-full border rounded px-2 py-1">
                @error('modelo_impresora') <span class="text-red-600">{{ $message }}</span> @enderror
            </div>

            <div>
                <label class="block font-medium">Color</label>
                <input type="text" name="color" value="{{ old('color', $consumible->color) }}" class="w-full border rounded px-2 py-1">
            ...
            <div>
                ...
                <input type="text" name="tipo_material" value="{{ old('tipo_material', $consumible->tipo_material) }}" class="w-full border rounded px-2 py-1">
